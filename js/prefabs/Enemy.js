...
princessPersis.Enemy.prototype.update = function (){
    ...
    var isThisAlive = this.isAlive;
    this.game.physics.arcade.collide(this, this.ground, function(){}, function(){if (isThisAlive){return true;}return false;});
...
if (this.player != null)
{
    ...
    if (this.followPlayer && princessPersis.customParams.isPlayerAlive == true && this.isAlive) 
        {
            if (this.x > player.x) {
     ...
     if (this.player.playerLeft == false) 
            {
                    if (this.onRightOfPlayer== true && this.touchingPlayer == true && princessPersis.customParams.isPlayerPunching == true && this.isAlive && princessPersis.customParams.isPlayerAlive) 
                        {
                            var hitSound = this.game.add.audio('hitSound');
                            hitSound.play();
            ...
            if ( this.player.playerLeft == true)
            {
                    if (this.onRightOfPlayer== false && Math.abs(distanceBetweenPlayerX(this.player,this)<70) && Math.abs(distanceBetweenPlayerY(this.player, this)) < 50.00  && princessPersis.customParams.isPlayerPunching == true && this.isAlive && princessPersis.customParams.isPlayerAlive) 
                        {
                            this.gameOverSound = princessPersis.game.add.audio('hitSound');
    ...
    if (this.y >= 700) {this.health = 0; this.kill()}
    if (this.health <=0)
        {
//            var fallDirection
//            if (this.x > player.x) {fallDirection = 1 } else {fallDirection=-1}
            
//            if (this.player.playerLeft && this.deathToggle) {var angleDirection = -1} 
//            if (!this.player.playerLeft && this.deathToggle) {var angleDirection =1}
//            console.log(angleDirection + "is the value of angleDirection");
        //this.kill(); 
        this.isAlive = false;
            //if (angleDirection) {this.angle += 5 * angleDirection;} else {this.angle += 5}
            this.angle += 5;
            this.body.gravity.y=2000;
            //this.body.velocity.x = 200 * angleDirection;
