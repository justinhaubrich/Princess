    ...
    function create() {
       ...
       testEnemy.isAlive = true;
        ...
        debugText2 = game.add.text(215, 16, 'debugText2', {fontsieze: '32', fill: '#FFF'});
    ...
    function update() {
       ...
       // if (timeCheck > game.time.now) {debugText.destroy()}; 
        ...
        if (cursors.up.isDown && player.body.touching.down)
            ...
            if (punchTime + 250 < game.time.now) {isPlayerPunching= false;}
            //player punches enemy
            if (isPlayerTouchingEnemy == true && isPlayerPunching == true && testEnemy.isAlive && isPlayerAlive) {testEnemy.kill(); testEnemy.isAlive = false;}
            //player gets attacked
            if (isPlayerTouchingEnemy == true && isPlayerPunching == false && testEnemy.isAlive) {player.kill(); isPlayerAlive = false}
