...
/*----------------------------------------------------------------------------------------------*/
    update: function() {
        if (princessPersis.customParams.introOn) 
        {
            ...
            //princessPersis.levelTwo.playerTween.onComplete.add(princessPersis.levelTwo.turnIntroOff, this);
            //this.playerTween._onCompleteCallback= this.turnIntroOff;
            //this.turnIntroOff();
            if (princessPersis.levelTwo.introPlayer.x >= 30){this.turnIntroOff();}
        } 
        else{
        //move the moving floor
        //princessPersis.game.state.states.levelTwo.movingBrickFloor.x = 500;
        ...
        if (player.x >= princessPersis.loadLevel.randomWorldSize)
            {
                princessPersis.game.state.start('levelTwo', true, false);
            }
    ...
    turnIntroOff: function()
    {
        ...
        //add powerups to level
         this.powerupOne = new princessPersis.HealthPowerup(playerHealth, player, princessPersis.game, 2900,200, this.brickSquare);
    ...
    {
        this.currentArrow;
        if (this.currentArrow === undefined || princessPersis.levelTwo.arrowGroup.getFirstExists(false) == null) 
        {
...
//    else if(sprite.direction ==-1){ }
    ...
    //sprite.directionVelocity = (xVelocity) * sprite.direction;  
    },
