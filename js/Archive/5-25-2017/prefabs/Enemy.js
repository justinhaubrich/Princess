...
princessPersis.Enemy = function (game, x, y, key, health, player, ground, arrowGroup) {
    ...
    {
        if (arrow.body.velocity.x <= 200) {arrow.body.gravity.y = 1600}
        else {
        this.health = 0;
...
princessPersis.Enemy.prototype.update = function (){
    ...
    if (Math.abs(distanceBetweenPlayerX(player, this)) < 25.00 &&       Math.abs(distanceBetweenPlayerY(player, this)) < 50.00)                     
        {
    ...
    //player punches this
     if (player.playerLeft == false) 
            ...
            if ( player.playerLeft == true)
            {
                    if (this.onRightOfPlayer== false && Math.abs(distanceBetweenPlayerX(player,this)<70) && Math.abs(distanceBetweenPlayerY(player, this)) < 50.00  && princessPersis.customParams.isPlayerPunching == true && this.isAlive && princessPersis.customParams.isPlayerAlive) 
                        {
    ...
    if (this.touchingPlayer == true && princessPersis.customParams.isPlayerPunching == false && this.isAlive)
                {
                    ...
                    if (princessPersis.GameState.playerHealth <=0 && player.alive)
                            {
                            var emitter = this.game.add.emitter(player.x + 50, player.y +15, 1200);
                            emitter.makeParticles('blood');
                            emitter.minParticleSpeed.setTo(-200,-200);
                            emitter.maxParticleSpeed.setTo(200,200);
                            emitter.gravity = 600;
